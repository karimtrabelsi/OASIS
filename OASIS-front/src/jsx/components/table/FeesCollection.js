...
const setUserr = JSON.parse(localStorage.getItem("connectedUser"));
   useEffect(() => {
      axios
         .post("http://localhost:3000/Event-Table")
         .then((res) => {
            setEvents(res.data);
         })
         .catch((err) => {
...
<Fragment>
    ...
    preConfirm: () => {
      ...
      const axes = Array.from(document.querySelectorAll('select[name="axes"] option:checked')).map(e => e.value); // sélectionner les options cochées du select
      const currentDate = new Date().toISOString().split('T')[0];
                               ...
                               if (!eventname || !startDate || !enddate || !place || !collaborateur || !typeEvent || !axes) {
                                ...
                                Swal.showValidationMessage('End date must be after start date');
                                 ...
                                 return { eventname: eventname, startDate: startdate,  enddate: enddate, place: place, collaborateur: collaborateur, typeEvent: typeEvent, axes:axes };
                                   ...
                                   fetch('http://localhost:3000/Table-Event/newEvent', {
                                      method: 'POST',
                                      headers: {
                                         'Content-Type': 'application/json'
                                      },
                                      body: JSON.stringify( newEvent/*result.value*/)
                                   })
                                   .then(response => {
                                    if (!response.ok) {
  ...
  {showDetails[event._id] ? 'Hide Details' : 'Show Details'}
            ...
            <Button variant="primary" onClick={() => {
              swal({
                ...
                if (willDelete) {
                  axios.delete("http://localhost:3000/deletEvent/" + event._id)
                    .then((res) => {
                      if (res.status === 200) {
