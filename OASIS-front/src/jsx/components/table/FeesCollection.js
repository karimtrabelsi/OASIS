...
const FeesCollection = () => {
  ...
  useEffect(() => {
    ...
    search &&
      setEvents(events.filter((event) => event.eventname.includes(search)));
  }, [events]);
                ...
                onClick={() =>
                  Swal.fire({
                    ...
                    if (result.isConfirmed) {
                      ...
                      const typeEvent = document
                        .getElementById("typeEvent")
                        .addEventListener("change", function () {
                          alert("Vous avez sélectionné " + this.value);
                        });
                      const axes = Array.from(
                        document.querySelectorAll('input[name="axes"]:checked')
                      ).map((e) => e.value);
                  ...
                  accumulator[accumulator.length - 1].push(
                    <Card key={event._id} className="my-3">
                      ...
                      <Card.Body>
                        ...
                        <Card.Text>
                          Collaborater : {event.collaborateur}
                        </Card.Text>
